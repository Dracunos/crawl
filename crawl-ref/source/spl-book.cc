...
bool player_can_memorise(const item_def &book)
{
    ...
    for (spell_type stype : spells_in_book(book))
    {
        ...
        if (spell_difficulty(stype) > you.experience_level + 1
                          ...
                          spell_type current_spell = SPELL_NO_SPELL)
{
    ...
    for (const spell_type spell : available_spells)
    {
        ...
        else
        {
            ...
            if (spell_difficulty(spell) > you.experience_level + 1)
                num_low_xl++;
                                    ...
                                    unsigned int num_misc)
{
    ...
    for (unsigned int i = 0; i < spells.size(); i++)
    {
        ...
        else if (spell_difficulty(spell) > you.experience_level + 1
...
static bool _learn_spell_checks(spell_type specspell, bool wizard = false)
{
    ...
    if (you.experience_level + 1 < spell_difficulty(specspell) && !wizard)
    {
